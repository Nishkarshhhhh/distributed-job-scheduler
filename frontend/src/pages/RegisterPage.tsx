...
export function RegisterPage() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      setError(details?.[0]?.message ?? err.response?.data?.message ?? "Registration failed");
    } finally {
    ...
    <div className="min-h-screen flex items-center justify-center px-4 bg-bg relative overflow-hidden">
      ...
      <div className="relative w-full max-w-sm">
        ...
        <div className="bg-surface border border-border rounded-2xl shadow-card p-8">
          <h1 className="font-display text-xl font-bold text-text mb-1">Create an account</h1>
          <p className="text-sm text-text-muted mb-6">Start scheduling background jobs</p>
